...
export const getRandomWord = () => {
  try {
    ...
    const randomWord = getRandomN5Word()
    usedWords.push(randomWord)

    // Obtener tres opciones incorrectas aleatorias
    const incorrectOptions = []
    while (incorrectOptions.length < 3) {
      const option = getRandomN5Word()
      if (option.word !== randomWord.word && !incorrectOptions.includes(option)) {
    ...
    return {
      ...
      explanation: `La palabra "${randomWord.word}" (${randomWord.reading}) significa "${randomWord.translation}" en español.`
    }
  } catch (error) {
